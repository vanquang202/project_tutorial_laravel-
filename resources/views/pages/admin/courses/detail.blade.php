...
@section('page-script')
    <script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.status) {
                        ...
                        if (data.errors) {
                            var html = data.errors.image.map(function(error) {
                                return `
        ...
        $('.file-upload-change').on('change', function() {
            ...
            uploadFile('https://api.laravel.org/api/update-image-course/' + course_id, formData, key);
        });
        $('#upload-file').on('change', function() {
            ...
            formData.append('image', $(this)[0].files[0]);
            formData.append('_token', _token);
            uploadFile('https://api.laravel.org/api/upload-image-course/' + course_id, formData, -1);
        });
    </script>
@endsection
